  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Nunito', sans-serif" }}>
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 24, padding: "52px 48px", width: 420, textAlign: "center", boxShadow: "0 8px 40px rgba(0,0,0,0.07)" }}>
        ...
        {error && (
          <div style={{ background: T.overdue.bg, border: `1px solid ${T.overdue.border}`, borderRadius: 14, padding: "14px 18px", fontSize: 13, color: T.overdue.fg, marginBottom: 16, lineHeight: 1.5 }}>
            {error}
          </div>
        )}

        {done ? (
          <div style={{ background: T.graded.bg, border: `1px solid ${T.graded.border}`, borderRadius: 14, padding: "16px 20px", fontSize: 15, color: T.graded.fg, fontWeight: 700 }}>
            ✓ Signed in! Setting up your account…
          </div>
        ) : (
          <Btn variant="google" onClick={handleSignIn} disabled={loading} style={{ width: "100%", justifyContent: "center", padding: "13px 24px", fontSize: 15, borderRadius: 14 }}>
            {loading ? (
              <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ width: 16, height: 16, border: "2px solid #dadce0", borderTopColor: T.accent, borderRadius: "50%", animation: "spin 0.7s linear infinite", display: "inline-block" }} />
                Signing in…
              </span>
            ) : (
              <>
        ...
        <div style={{ marginTop: 28, padding: "16px 20px", background: T.accentBg, borderRadius: 12, fontSize: 13, color: T.accent, lineHeight: 1.6 }}>
    ...
    return (
      <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Nunito', sans-serif" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ width: 48, height: 48, border: `4px solid ${T.border}`, borderTopColor: T.accent, borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 20px" }} />
          <div style={{ fontSize: 16, fontWeight: 600, color: T.text }}>Loading your classroom...</div>
        </div>
        <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
      </div>
    );
